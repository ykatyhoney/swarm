...
angular.module('swarmApp').controller('LoadSaveCtrl', function($scope, $log, game, session, version, $location, backfill, isKongregate, storage, saveId) {
  ...
  try {
    ...
    if (!exportedsave) {
      ...
      storage.flash.onReady.then(function() {
        const encoded = storage.flash.getItem(saveId);
        if (encoded) {
          ...
          return game.importSave(encoded, true); // don't save when recovering from flash - if this is somehow a mistake, player can take no action
        } else {
          return $log.debug('flash loaded successfully, but no saved game found. this is truly a new visitor.');
        }
      });
    } else {
...
angular.module('swarmApp').controller('WelcomeBackCtrl', function($scope, $log, $interval, game, $location) {
  ...
  return (run = function(force, ignoreHeartbeat) {
    ...
    let leaders = 0;
    for (let unit of Array.from(game.tabs.byName.meat.sortedUnits)) {
      if (leaders >= 3) {
        break;
      }
