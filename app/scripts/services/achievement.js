...
return Achievement = class Achievement {
  ...
  isUnmasked() {
    ...
    for (let visible of Array.from(this.visible)) {
  ...
  hasProgress() {
    for (let req of Array.from(this.requires)) {
...
return AchievementsListener = class AchievementsListener {
  ...
  achieveUnit(unitname, rawcount) {
    ...
    return Array.from(this.game.achievementlist()).map((achieve) =>
      (() => {
        const result = [];
        for (let require of Array.from(achieve.requires)) {
      ...
      (() => {
        ...
        for (let require of Array.from(achieve.requires)) {
          if (!require.event && require.upgrade && require.val) {
            ...
            const count = require.upgrade.count();
  ...
  _listen(scope) {
    this.scope = scope;
    for (let achieve of Array.from(this.game.achievementlist())) { (achieve => {
      return (() => {
        const result = [];
        for (let require of Array.from(achieve.requires)) {
