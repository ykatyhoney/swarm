...
angular.module('swarmApp').factory('Upgrade', function(util, Effect, $log) {
  ...
  return Upgrade = class Upgrade {
    ...
    costMetPercent() {
      ...
      for (let cost of Array.from(this.sumCost(this.maxCostMet().plus(1)))) {
        const count = cost.unit.count().minus(costOfMet[cost.unit.name].val);
        const val = cost.val.minus(costOfMet[cost.unit.name].val);
    ...
    _estimateSecsUntilBuyable() {
      ...
      for (let cost of Array.from(this.sumCost(this.maxCostMet().plus(1)))) {
        const secs = cost.unit.estimateSecsUntilEarned(cost.val);
    ...
    buy(num) {
      ...
      return this.game.withSave(() => {
        for (let cost of Array.from(this.sumCost(num))) {
...
return UpgradeTypes = class UpgradeTypes {
  constructor(unittypes, upgrades) {
    ...
    this.list = [];
    this.byName = {};
    for (let upgrade of Array.from(upgrades)) {
  ...
  static parseSpreadsheet(unittypes, effecttypes, data) {
    ...
    for (let upgrade of Array.from(ret.list)) {
      ...
      for (let cost of Array.from(upgrade.cost)) {
    ...
    for (let unittype of Array.from(unittypes.list)) {
