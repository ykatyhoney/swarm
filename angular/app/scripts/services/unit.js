...
return ProducerPath = class ProducerPath {
  ...
  prodEach() {
    return this.unit.game.cache.producerPathProdEach[this.name] != null ? this.unit.game.cache.producerPathProdEach[this.name] : (this.unit.game.cache.producerPathProdEach[this.name] = (() => {
      ...
      for (let ancestordata of Array.from(this.path)) {
        const val = new Decimal(ancestordata.prod.val).plus(ancestordata.parent.stat('base', 0));
        ret = ret.times(val);
...
return ProducerPaths = class ProducerPaths {
  ...
  getDegreeCoefficient(degree, now) {
    ...
    for (let path of Array.from(this.byDegree[degree] != null ? this.byDegree[degree] : [])) {
...
return Unit = (function() {
  Unit = class Unit {
    ...
    _parents() {
      return (() => {
        const result = [];
        for (let pathdata of Array.from(this._producerPath.list)) {           if (pathdata.first().parent.prodByName[this.name]) {
    ...
    spentResources() {
      return (() => {
        const result = [];
        for (let u of Array.from([].concat(this.game.unitlist(), this.game.upgradelist()))) {           if (u.costByName[this.name] != null) {
            result.push(u);
          }
        }
        return result;
    ...
    spent(ignores) {
      ...
      for (u of Array.from(this.game.unitlist())) {
        const costeach = (u.costByName[this.name] != null ? u.costByName[this.name].val : undefined) != null ? (u.costByName[this.name] != null ? u.costByName[this.name].val : undefined) : 0;
      ...
      for (u of Array.from(this.game.upgradelist())) {
        if (u.costByName[this.name] && (ignores[u.name] == null)) {
          ...
          const costs = u.sumCost(u.count(), 0);
          const cost = _.find(costs, c => c.unit.name === this.name);
    ...
    _costMetPercent() {
      ...
      for (let cost of Array.from(this.eachCost())) {
    ...
    _isVisible() {
      ...
      for (let require of Array.from(this.requires)) {
    ...
    isBuyButtonVisible() {
      ...
      for (let cost of Array.from(eachCost)) {
    ...
    buy(num) {
      ...
      return this.game.withSave(() => {
        ...
        for (let effect of Array.from(this.effect)) {
    ...
    eachProduction() {
      return this.game.cache.eachProduction[this.name] != null ? this.game.cache.eachProduction[this.name] : (this.game.cache.eachProduction[this.name] = (() => {
        const ret = {};
        for (let prod of Array.from(this.prod)) {
    ...
    stats() {
      return this.game.cache.stats[this.name] != null ? this.game.cache.stats[this.name] : (this.game.cache.stats[this.name] = (() => {
        const stats = {};
        const schema = {};
        for (let upgrade of Array.from(this.upgrades.list)) {
          upgrade.calcStats(stats, schema);
        }
        for (let uniteffect of Array.from(this.affectedBy)) {
...
return UnitTypes = class UnitTypes {
  constructor(unittypes) {
    ...
    this.list = [];
    this.byName = {};
    for (let unittype of Array.from(unittypes)) {
