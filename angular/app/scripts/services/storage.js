...
angular.module('swarmApp').factory('flashStorage', function($q, $log, env) { let FlashStorage;
return new (FlashStorage = class FlashStorage {
  constructor() {
    ...
    try {
      this.storage = new SwfStore({
        ...
        timeout: 10,
        onready: () => {
          if (this.isReady == null) { this.isReady = true; }
          deferred.resolve();
          return $log.debug('flash storage ready');
        },
        onerror: () => {
          this.isReady = false;
          deferred.reject();
          return $log.warn('flash storage error');
        }
      });
    } catch (e) {
      this.isReady = false;
...
return MultiStorage = class MultiStorage {
  ...
  _withEachStore(name, fn, pred) {
    ...
    let errorcount = 0;
    for (let store of Array.from(this.storages.list)) {
      try {
        const ret = fn(store);
        if (pred(ret)) {
          return ret;
...
angular.module('swarmApp').factory('storage', (MultiStorage, flashStorage, cookieStorage) => new MultiStorage()
